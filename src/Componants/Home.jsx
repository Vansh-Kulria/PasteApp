...
const Home = () => {
    ...
    useEffect(() => {
        ...
        setError(''); // Clear error when pasteId changes
    }, [pasteId])

    function createPaste() {
        // if title is empty then highlight the title input field and show error message
        if (!title.trim()) {
    ...
    return (
        ...
        bg-gradient-to-br from-blue-100 via-blue-200 to-blue-300">
            <div className="bg-white/80 backdrop-blur-lg shadow-2xl rounded-2xl p-4 w-full max-w-2xl min-h-[300px] max-h-[calc(100vh-100px)] border border-blue-200 mt-1.5 flex flex-col mx-2 ">
                ...
                <div className="flex flex-col gap-3">
                    <input
                        className={`title px-4 py-2 rounded-lg border ${error ? 'border-red-500' : 'border-blue-300'} focus:outline-none focus:ring-2 focus:ring-blue-400 transition-all text-lg shadow`}
                        type="text"
                        placeholder={error ? error : "Enter title here"}
                        value={title}
                        onChange={(e) => {
